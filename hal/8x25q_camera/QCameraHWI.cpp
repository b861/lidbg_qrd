...
namespace android
{
    ...
    void QCameraHardwareInterface::stopPreview()
    {
        ...
        if(NULL != mStreamSnap)
            ((QCameraStream_Snapshot *)mStreamSnap)->releaseExtBuffer();
    }
    ...
    */
    ...
    {
        int32_t enabled_again = 0;
        int32_t enabled = 0;
        int frm_num;
        uint32_t  skip_mode;
        char value[PROPERTY_VALUE_MAX];
        char value_agian[PROPERTY_VALUE_MAX];
        char buf[32];
        int main_422 = 1;
        static int dump_image_count = 1;
        property_get("persist.camera.dumpimg", value, "0");
        property_get("persist.camera.dumpimg.gain", value_agian, "0");
        enabled = atoi(value);
        //enabled=0 ;
        enabled_again = atoi(value_agian);
        //enabled = HAL_DUMP_FRM_PREVIEW;
        //ALOGV(" newFrame =%p, frm_type = %d", newFrame, frm_type);
        if(enabled_again == 1)
        {
            mDumpFrmCnt = 1;
            dump_image_count ++;
            if(dump_image_count > 20)dump_image_count = 0;
            property_set("persist.camera.dumpimg.gain", "0");
            ALOGE("dumpFrameToFile_again\n");
        }
                ...
                if( mDumpSkipCnt % skip_mode == 0)
                {
                    if (mDumpFrmCnt >= 0 && mDumpFrmCnt <= frm_num)
                    {
                        ...
                        case  HAL_DUMP_FRM_PREVIEW:
                            ...
                            if(enabled_again == 1)
                                snprintf(buf, sizeof(buf), "/data/%d_%dx%d.yuv", dump_image_count, w, h);
                            else
                                snprintf(buf, sizeof(buf), "/data/%dp_%dx%d.yuv", mDumpFrmCnt, w, h);
                            file_fd = open(buf, O_RDWR | O_CREAT, 0777);
                            break;
