...
namespace android
{
    ...
    void QCameraStream_Snapshot::setScalingUpMode()
    {
        if(isLiveSnapshot() || isFullSizeLiveshot())
        {
            if(mHalCamCtrl->mDimension.picture_width > mHalCamCtrl->mActualSensorWidth ||
                    mHalCamCtrl->mDimension.picture_height > mHalCamCtrl->mActualSensorHeight)
            {
                mHalCamCtrl->mUISetCameraWidth = mHalCamCtrl->mActualSensorWidth ;
                mHalCamCtrl->mUISetCameraHeight = mHalCamCtrl->mActualSensorHeight;
                mHalCamCtrl->mDimension.picture_width  = mHalCamCtrl->mActualSensorWidth;
                mHalCamCtrl->mDimension.picture_height = mHalCamCtrl->mActualSensorHeight;
            }
            mHalCamCtrl->mCaptureScalingUp = false ;
            return ;
        }
        if(mHalCamCtrl->mCameraResolutionUpSupport) //if support scalingup
        {
            mHalCamCtrl->mUISetCameraWidth = mHalCamCtrl->mDimension.picture_width;
            mHalCamCtrl->mUISetCameraHeight = mHalCamCtrl->mDimension.picture_height;
            ALOGW("Max Sensor output size = : %dx%d",
                  mHalCamCtrl->mActualSensorWidth, mHalCamCtrl->mActualSensorHeight);
            ALOGW("UI setting output size = : %dx%d",
                  mHalCamCtrl->mUISetCameraWidth, mHalCamCtrl->mUISetCameraHeight);
            if(mHalCamCtrl->mDimension.picture_width > mHalCamCtrl->mActualSensorWidth ||
                    mHalCamCtrl->mDimension.picture_height > mHalCamCtrl->mActualSensorHeight)
            {
                mHalCamCtrl->mCaptureScalingUp = true;
                mHalCamCtrl->mDimension.picture_width = mHalCamCtrl->mActualSensorWidth;
                mHalCamCtrl->mDimension.picture_height = mHalCamCtrl->mActualSensorHeight;
            }
            else
    ...
    initSnapshotBuffers(cam_ctrl_dimension_t *dim, int num_of_buf)
    {
        ...
        mm_camera_reg_buf_t reg_buf;
        int rotation = 0;
        uint32_t frame_len_scalingup  ;
        uint32_t y_off_scalingup ;
        uint32_t cbcr_off_scalingup ;
        uint32_t sensor_width, sensor_height;
        uint32_t ui_set_picture_width, ui_set_picture_height;
        setScalingUpMode() ;
        sensor_width = mHalCamCtrl->mActualSensorWidth ;
        sensor_height = mHalCamCtrl->mActualSensorHeight;
        ui_set_picture_width = mHalCamCtrl->mUISetCameraWidth;
        ui_set_picture_height = mHalCamCtrl->mUISetCameraHeight;
        ALOGW("wk_log:ui_resolution,sensor_resolution", ui_set_picture_width, ui_set_picture_height, sensor_width, sensor_height) ;
        ...
        if(!isLiveSnapshot())
        {
            if(mHalCamCtrl->mCaptureScalingUp) //if need scaling up, allocate pmemory for temp ;
            {
                frame_len_scalingup = PAD_TO_4K(PAD_TO_4K (CEILING16(ui_set_picture_width) * CEILING16(ui_set_picture_height) * 3 / 2) );
                ALOGW("wk_log_mem:allocat:%d", frame_len_scalingup) ;
                if(mHalCamCtrl->initHeapMem(&mHalCamCtrl->mScalingUpMemory, 1,
                                            frame_len_scalingup, y_off, cbcr_off, MSM_PMEM_MAINIMG, &mScalingUpStreamBuf,
                                            &reg_buf.snapshot.main, num_planes, planes) < 0)
                {
                    ret = NO_MEMORY;
                    ALOGD("wk_log:%s:initHeapMem:No_memory", __func__) ;
                    mHalCamCtrl->releaseHeapMem(&mHalCamCtrl->mScalingUpMemory);
                    goto end;
                }
            }
    ...
    deinitSnapshotBuffers(void)
    {
        ...
        if (getSnapshotState() >= SNAPSHOT_STATE_BUF_INITIALIZED )
        {
            ...
            if(mHalCamCtrl->mCaptureScalingUp)
            {
                mHalCamCtrl->releaseHeapMem(&mHalCamCtrl->mScalingUpMemory);
            }
        }
    ...
    static int count = 0 ;
    void dumpfile(struct msm_frame *recvd_frame, uint32_t width, uint32_t height)
    {
        int file_fd;
        char buf[32];
        count++ ;
        snprintf(buf, sizeof(buf), "/data/scaling%dx%d_%d.yuv", width, height, count);
        file_fd = open(buf, O_RDWR | O_CREAT, 0777);
        write(file_fd, (const void *)(recvd_frame->buffer + recvd_frame->y_off), width * height);
        write(file_fd, (const void *)
              (recvd_frame->buffer + recvd_frame->cbcr_off), width * height / 2 );
        close(file_fd);
    }
    static bool scaleUp(struct msm_frame *dst_frame, struct msm_frame *src_frame,
                        uint32_t dst_width, uint32_t dst_height, uint32_t src_width, uint32_t src_height)
    {
        if(dst_frame == NULL || src_frame == NULL)
        {
            ALOGW("%s:input parameters invalid", __func__) ;
            return false ;
        }
        ...
        blitReq->src.width		= src_width;
        blitReq->src.height = src_height;
        blitReq->src.format = MDP_Y_CRCB_H2V2;
        blitReq->src.offset = src_frame->phy_offset;
        blitReq->src.memory_id = src_frame->fd;
        blitReq->dst.width		= dst_width;
        blitReq->dst.height = dst_height;
        blitReq->dst.format = MDP_Y_CRCB_H2V2;
        blitReq->dst.offset = dst_frame->phy_offset;
        blitReq->dst.memory_id = dst_frame->fd;
        blitReq->transp_mask = 0xffffffff;
        blitReq->flags = 0;
        blitReq->alpha	= 0xFF;
        blitReq->dst_rect.x = 0;
        blitReq->dst_rect.y = 0;
        blitReq->dst_rect.w = dst_width;
        blitReq->dst_rect.h = dst_height;
        blitReq->src_rect.x = 0;
        blitReq->src_rect.y = 0;
        blitReq->src_rect.w = src_width;
        blitReq->src_rect.h = src_height;

        if((fb_fd = open("/dev/graphics/fb0", O_RDWR, 0)) < 0)
        {
            ALOGE("%s:open graphics fb0 failed!", __func__) ;
            fb_fd = -1 ;
            return false ;
        }
        if (ioctl(fb_fd, MSMFB_BLIT, &imgFrame.list))
        {
            ALOGE("%s:ioctl,execute scale up failed!", __func__) ;
            close(fb_fd) ;
            fb_fd = -1 ;
            return false ;
        }
               ...
               bool enqueued)
    {
        ...
        ui_set_picture_width = mHalCamCtrl->mUISetCameraWidth;
        ui_set_picture_height = mHalCamCtrl->mUISetCameraHeight;
        uint8_t b_zooming = false ;
        struct msm_frame *scaling_dst = &(mScalingUpStreamBuf.frame[0]) ;
        ALOGW("%s:E", __func__) ;
        ALOGW("%s:sensor_reso=%dx%d,ui_reso=%dx%d", __func__, sensor_width, sensor_height, ui_set_picture_width, ui_set_picture_height) ;
                 ...
                 (mNumOfRecievedJPEG != mNumOfSnapshot  && mNumOfRecievedJPEG != 0))   /*not busy, not first*/
        {
            ...
            ALOGW("wk_log:%s:origin dimension:%dx%d,picture reso:%dx%d", __func__, dimension.orig_picture_dx, dimension.orig_picture_dy,
            ...
            if(!b_zooming && mHalCamCtrl->mCaptureScalingUp) //here we also need to scale up
            {
                encode_params.thumbnail_buf = (uint8_t *)postviewframe->buffer;
                encode_params.thumbnail_fd = postviewframe->fd;
                scaleUp(scaling_dst, mainframe, ui_set_picture_width, ui_set_picture_height, sensor_width, sensor_height) ;
                encode_params.snapshot_buf = (uint8_t *)scaling_dst->buffer;
                encode_params.snapshot_fd = scaling_dst->fd;
            }
            else
            {
                encode_params.thumbnail_buf = (uint8_t *)postviewframe->buffer;
        ...
        else      /*not busy and new buffer (first job)*/
        {
            ...
            ALOGW("wk_log:%s:origin dimension:%dx%d,picture reso:%dx%d", __func__, dimension.orig_picture_dx, dimension.orig_picture_dy,
                  dimension.picture_width, dimension.picture_height) ;
            ALOGW("wk_log:%s:mPicture %dx%d", __func__, mPictureWidth, mPictureHeight) ;
            ...
            if(crop.in2_w && crop.in2_h)
            {
                ...
                if(mHalCamCtrl->mCaptureScalingUp)
                {
                    mPictureWidth = ui_set_picture_width;
                    mPictureHeight = ui_set_picture_height;
                }
            }
            if (!mJpegDownscaling)
            {
            ...
            if(!b_zooming)
            {
                ...
                {
                    scaleUp(scaling_dst, mainframe, ui_set_picture_width, ui_set_picture_height, sensor_width, sensor_height) ;
                    dimension.orig_picture_dx = ui_set_picture_width;
                    dimension.orig_picture_dy = ui_set_picture_height;
                }
            }
            else
            {
                if( mHalCamCtrl->mCaptureScalingUp)
                {
                    dimension.orig_picture_dx = sensor_width;
                    dimension.orig_picture_dy = sensor_height;
                }
            }
            ...
            //}
            if(!b_zooming && mHalCamCtrl->mCaptureScalingUp)
            {
                encode_params.snapshot_buf = (uint8_t *)scaling_dst->buffer;
                encode_params.snapshot_fd = scaling_dst->fd;
                encode_params.snapshot_offset = scaling_dst->phy_offset;
            }
            else
            {
                encode_params.snapshot_buf = (uint8_t *)mainframe->buffer;
            ...
            if (isLiveSnapshot() && !isFullSizeLiveshot())
            {
                if(!b_zooming && mHalCamCtrl->mCaptureScalingUp)
                {
                    encode_params.a_cbcroffset = scaling_dst->cbcr_off;
                }
                else
                {
    ...
    status_t QCameraStream_Snapshot::receiveRawPicture(mm_camera_ch_data_buf_t *recvd_frame)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                QCameraParameters parameters = mHalCamCtrl->mParameters;
                const char *pMode = parameters.get(ts::KEY_TS_EXT_MODE);
                if (pMode != NULL && strcmp(pMode, ts::TS_EXT_MODE_NONE) != 0)
                {
                    mExtMode.SetCameraParameters((CameraParameters)parameters, mPictureWidth, mPictureHeight, mHalCamCtrl->mPreviewWidth, mHalCamCtrl->mPreviewHeight, mThumbnailWidth, mThumbnailHeight, mHalCamCtrl->getJpegRotation(), mHalCamCtrl->mCameraId);
                    mExtMode.Process(frame->snapshot.main.frame, frame->snapshot.thumbnail.frame);
                    mExtMode.ReleaseBuffer((CameraParameters)parameters);
                }
    ...
    void QCameraStream_Snapshot::releaseExtBuffer()
    {
        mExtMode.ReleaseBuffer((CameraParameters)mHalCamCtrl->mParameters);
    }
    ...
    status_t QCameraStream_Snapshot::start(void)
    {
        ...
        if (isZSLMode())
        {
            ...
            ret = startStreamZSL();
            //Workaround for ZSL+RAW, disable RAW format when goes into ZSL
            mSnapshotFormat = PICTURE_FORMAT_JPEG;
            goto end;
        }
