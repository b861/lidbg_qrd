...
#define DONT_CARE AF_MODE_MAX
    ...
    void QCameraHardwareInterface::initDefaultParameters()
    {
        ...
        if (true/*!mParamStringInitialized*/)
        {
            ...
            else
            {
                ...
                //Add the fps range value which was got from sensor into mSupportedFpsRanges table,
                //if it's not in the table yet.
                int addPosition = mSupportedFpsRangesCount;
                for (int i = 0; i < mSupportedFpsRangesCount; i++)
                {
                    ...
                    else if (mSupportedFpsRanges[i].maxFPS == maxFps)
                    {
                        ...
                        {
                            //Already in mSupportedFpsRanges table, no need to add.
                            addPosition = -1;
                            break;
                        }
                    }
                ...
                //Set the default fps as the value got from sensor.
        ...
        mParameters.set(QCameraParameters::KEY_QC_MAX_NUM_REQUESTED_FACES, 5);
        ...
        {
            ALOGE("Support HFR parm, enable HFR mode.");
        ...
        {
            ALOGE("Don't support HFR parm, disable HFR mode.");
            mParameters.set(QCameraParameters::KEY_QC_SUPPORTED_HFR_SIZES, "");
        }
        ...
        if(cam_config_is_parm_supported(mCameraId, MM_CAMERA_PARM_HISTOGRAM))
        {
        ...
        if(cam_config_is_parm_supported(mCameraId, MM_CAMERA_PARM_BESTSHOT_MODE))
        {
        ...
        if(cam_config_is_parm_supported(mCameraId, MM_CAMERA_PARM_FOCUS_RECT))
        {
        ...
        if(cam_config_is_parm_supported(mCameraId, MM_CAMERA_PARM_REDEYE_REDUCTION))
        {
        ...
        //Set ext mode
        mParameters.set(ts::KEY_TS_EXT_MODE, ts::TS_EXT_MODE_NONE);
        // ext-mode magiclens param
        mParameters.set(ts::KEY_TS_EXT_VERTEX_SHADER, "none");
        mParameters.set(ts::KEY_TS_EXT_FRAGMENT_SHADER, "none");
        mParameters.set(ts::KEY_TS_EXT_MODE_PARAM, "none");
        // ext-mode pip param
        mParameters.set(ts::KEY_TS_EXT_PIP_PATH, "none");
        mParameters.set(ts::KEY_TS_EXT_PIP_PAINT, "none");
        mParameters.set(ts::KEY_TS_EXT_PIP_PICTURE_NUM, "0");
        ...
        mParameters.set("QRDCameraSDK", "true");
        // if(mIs3DModeOn)
    ...
    /**
    *change Camera UI if there are parameters conflict when camera on create
    */
    void QCameraHardwareInterface::changeUIOnCreate(const QCameraParameters &params)
    {
        mParameters.set("param_group_update", params.getInt("param_group_update"));

        const char *strNewHfr = params.get(QCameraParameters::KEY_QC_VIDEO_HIGH_FRAME_RATE);
        ALOGE("strNewHfr = %s;", strNewHfr);
        if (strNewHfr != NULL)
        {
            int valueNewHfr = attr_lookup(hfr, sizeof(hfr) / sizeof(str_map), strNewHfr);
            if (valueNewHfr != NOT_FOUND)
            {
                if (valueNewHfr != CAMERA_HFR_MODE_OFF)
                {
                    ALOGE("HFR is ON, remove unsupported video sizes.");
                    mParameters.set(QCameraParameters::KEY_SUPPORTED_VIDEO_SIZES, mHfrSizeValues.string());
                    mParameters.set("param_group_update", 1);
                }
                else if (valueNewHfr == CAMERA_HFR_MODE_OFF)
                {
                    ALOGE("HFR is OFF, add supported video sizes.");
                    mParameters.set(QCameraParameters::KEY_SUPPORTED_VIDEO_SIZES, mVideoSizeValues.string());
                    mParameters.set("param_group_update", 1);
                }
            }
        ...
        const char *strNewVs = params.get(QCameraParameters::KEY_VIDEO_SIZE);
        ALOGE("strNewVs = %s", strNewVs);
        if (strNewVs != NULL)
        {
            if (!supportVideoSizeOfHFR(params))
            {
                ALOGE("Video Quality unsupported, disable HFR mode.");
                mParameters.set(QCameraParameters::KEY_QC_SUPPORTED_VIDEO_HIGH_FRAME_RATE_MODES, "");
                mParameters.set("param_group_update", 1);
            }
            else if (supportVideoSizeOfHFR(params))
            {
                ALOGE("Video Quality supported, enable HFR mode.");
                mParameters.set(QCameraParameters::KEY_QC_SUPPORTED_VIDEO_HIGH_FRAME_RATE_MODES, mHfrValues);
                mParameters.set("param_group_update", 1);
            }
        }
    }
    ...
    /**
    *change Camera UI if there are parameters conflict
    */
    void QCameraHardwareInterface::changeUI(const QCameraParameters &params)
    {
        mParameters.set("param_group_update", params.getInt("param_group_update"));

        const char *strNewHfr = params.get(QCameraParameters::KEY_QC_VIDEO_HIGH_FRAME_RATE);
        const char *strOldHfr = mParameters.get(QCameraParameters::KEY_QC_VIDEO_HIGH_FRAME_RATE);
        ALOGE("strNewHfr = %s; strOldHfr = %s", strNewHfr, strOldHfr);
        if( (strNewHfr != NULL) && (strOldHfr != NULL) && strcmp(strNewHfr, strOldHfr) )
        {
            int valueNewHfr = attr_lookup(hfr, sizeof(hfr) / sizeof(str_map), strNewHfr);
            int valueOldHfr = attr_lookup(hfr, sizeof(hfr) / sizeof(str_map), strOldHfr);
            if ((valueNewHfr != NOT_FOUND) && (valueOldHfr != NOT_FOUND))
            {
                if ((valueOldHfr == CAMERA_HFR_MODE_OFF) && (valueNewHfr != CAMERA_HFR_MODE_OFF))
                {
                    if (supportVideoSizeOfHFR(params))
                    {
                        ALOGE("HFR mode changes, remove unsupported video sizes.");
                        mParameters.set(QCameraParameters::KEY_SUPPORTED_VIDEO_SIZES, mHfrSizeValues.string());
                    }
                    mParameters.set("param_group_update", 1);
                }
                else if ((valueOldHfr != CAMERA_HFR_MODE_OFF) && (valueNewHfr == CAMERA_HFR_MODE_OFF))
                {
                    ALOGE("HFR mode changes, add supported video sizes.");
                    mParameters.set(QCameraParameters::KEY_SUPPORTED_VIDEO_SIZES, mVideoSizeValues.string());
                    mParameters.set("param_group_update", 1);
                }
            }
        ...
        const char *strNewVs = params.get(QCameraParameters::KEY_VIDEO_SIZE);
        const char *strOldVs = mParameters.get(QCameraParameters::KEY_VIDEO_SIZE);
        ALOGE("strNewVs = %s; strOldVs = %s", strNewVs, strOldVs);
        if( (strNewVs != NULL) && (strOldVs != NULL) && strcmp(strNewVs, strOldVs) )
        {
            if (!supportVideoSizeOfHFR(params) && supportVideoSizeOfHFR(mParameters))
            {
                ALOGE("Video Quality changes, disable HFR mode.");
                mParameters.set(QCameraParameters::KEY_QC_SUPPORTED_VIDEO_HIGH_FRAME_RATE_MODES, "");
                mParameters.set("param_group_update", 1);
            }
            else if (supportVideoSizeOfHFR(params) && !supportVideoSizeOfHFR(mParameters))
            {
                ALOGE("Video Quality changes, enable HFR mode.");
                mParameters.set(QCameraParameters::KEY_QC_SUPPORTED_VIDEO_HIGH_FRAME_RATE_MODES, mHfrValues);
                mParameters.set("param_group_update", 1);
            }
        }
    ...
    status_t QCameraHardwareInterface::setParameters(const QCameraParameters &params)
    {
        ...
        if ((rc = setPipMode(params)))                      final_rc = rc;
        ...
        if ((rc = setExtModeValue(params)))                 final_rc = rc;
        if ((rc = setMagicLensParam(params)))               final_rc = rc;
        if ((rc = setPipParam(params)))                     final_rc = rc;
    ...
    */
    ...
    //
    status_t QCameraHardwareInterface::setExtModeValue(const QCameraParameters &params)
    {
        const char *str = params.get(ts::KEY_TS_EXT_MODE);
        if (str)
            mParameters.set(ts::KEY_TS_EXT_MODE, str);
        return NO_ERROR;
    }

    status_t QCameraHardwareInterface::setMagicLensParam(const QCameraParameters &params)
    {
        const char *str = params.get(ts::KEY_TS_EXT_MODE_PARAM);
        if (str)
            mParameters.set(ts::KEY_TS_EXT_MODE_PARAM, str);
        const char *vertex = params.get(ts::KEY_TS_EXT_VERTEX_SHADER);
        if (vertex)
            mParameters.set(ts::KEY_TS_EXT_VERTEX_SHADER, vertex);
        const char *fragment = params.get(ts::KEY_TS_EXT_FRAGMENT_SHADER);
        if (fragment)
            mParameters.set(ts::KEY_TS_EXT_FRAGMENT_SHADER, fragment);
        return NO_ERROR;
    }

    status_t QCameraHardwareInterface::setPipParam(const QCameraParameters &params)
    {
        const char *path = params.get(ts::KEY_TS_EXT_PIP_PATH);
        if (path)
            mParameters.set(ts::KEY_TS_EXT_PIP_PATH, path);
        const char *paint = params.get(ts::KEY_TS_EXT_PIP_PAINT);
        if (paint)
            mParameters.set(ts::KEY_TS_EXT_PIP_PAINT, paint);
        const char *num = params.get(ts::KEY_TS_EXT_PIP_PICTURE_NUM);
        if (num)
            mParameters.set(ts::KEY_TS_EXT_PIP_PICTURE_NUM, num);
        return NO_ERROR;
    }
    // end
    ...
    /**
    *When HFR is on, check supported video quality
    */
    bool QCameraHardwareInterface::supportVideoSizeOfHFR(const QCameraParameters &params)
    {
        int width;
        int height;
        const char *str = params.get(QCameraParameters::KEY_VIDEO_SIZE);
        if (str != NULL)
        {
            if(!parse_size(str, width, height))
            {
                for(int i = 0; i < HFR_TBL_MAX_SIZE; i++)
                {
                    if(width != default_hfr_sizes[i].width)
                    {
                        continue;
                    }
                    else
                    {
                        if(height == default_hfr_sizes[i].height)
                        {
                            return true;
                        }
                    }
    ...
    status_t QCameraHardwareInterface::setVideoSize(const QCameraParameters &params)
    {
        if (!supportVideoSizeOfHFR(params))
        {
            int32_t mDefaultHFR = (int32_t)CAMERA_HFR_MODE_OFF;
            native_set_parms(MM_CAMERA_PARM_HFR, sizeof(int32_t), (void *)&mDefaultHFR);
        }
        const char *str = NULL;
    ...
    status_t QCameraHardwareInterface::setPipMode(const QCameraParameters &params)
    {
        uint32_t value = 0;
        const char *pipmode = NULL;
        char value_pip[PROPERTY_VALUE_MAX];
        uint32_t setting_flag = 0;
        /* //TODO: extend the ctrl path form APP/framework to hal, and using the codes below.
            pipmode = params.get(QCameraParameters::KEY_QC_PIP_MODE);
            if (pipmode != NULL) {
                ALOGE("%s, pipmode: %s\n",__func__,pipmode);
                value = attr_lookup(pip_modes,
                    sizeof(pip_modes) / sizeof(str_map), pipmode);
            }
        */
        property_get("persist.camera.hal.pip", value_pip, "0");
        if(1 == (uint32_t)atoi(value_pip)) //change to normal mode
        {
            value = 0;
            setting_flag = 1;
        }
        else if(2 == (uint32_t)atoi(value_pip)) //change to pip mode
        {
            value = 1;
            setting_flag = 1;
        }
        ...
        {
            ALOGE("%s, value = %d\n", __func__, value);
            bool ret = native_set_parms(MM_CAMERA_PARM_PIP_MODE,
                                        sizeof(value), (void *)&value);
        }
        return NO_ERROR;
    }
    ...
    status_t QCameraHardwareInterface::setHighFrameRate(const QCameraParameters &params)
    {
        ...
        if (str != NULL)
        {
            ...
            if (value != NOT_FOUND)
            {
                if ((value == CAMERA_HFR_MODE_OFF) || ((value != CAMERA_HFR_MODE_OFF) && supportVideoSizeOfHFR(params)))
                {
    ...
    status_t QCameraHardwareInterface::setRedeyeReduction(const QCameraParameters &params)
    {
        ...
        if((strHDR != NULL) && (str != NULL))
        {
            int valueHDR = attr_lookup(hdr_bracket, sizeof(hdr_bracket) / sizeof(str_map), strHDR);
            int value = attr_lookup(redeye_reduction, sizeof(redeye_reduction) / sizeof(str_map), str);
            if ((valueHDR == NOT_FOUND) || (valueHDR == HDR_BRACKETING_OFF))
            {
                if (value != NOT_FOUND)
                {
                    int8_t temp = (int8_t)value;
                    ALOGE("%s: setting Redeye Reduction value of %s", __FUNCTION__, str);
            ...
            {
                int8_t temp = (int8_t)FALSE;
                ALOGE("%s: setting Redeye Reduction value of %s", __FUNCTION__, QCameraParameters::REDEYE_REDUCTION_DISABLE);
                mParameters.set(QCameraParameters::KEY_QC_REDEYE_REDUCTION, QCameraParameters::REDEYE_REDUCTION_DISABLE);
                native_set_parms(MM_CAMERA_PARM_REDEYE_REDUCTION, sizeof(int8_t), (void *)&temp);
                return NO_ERROR;
            }
        }
    ...
    status_t QCameraHardwareInterface::setPictureFormat(const QCameraParameters &params)
    {
        ...
        //if zsl mode, set the format to jpeg
        if(isZSLMode())
        {
            ALOGE("%s #### in ZSL mode, set picutre format to jpeg", __func__);
            mParameters.set(QCameraParameters::KEY_PICTURE_FORMAT,
                            QCameraParameters::PIXEL_FORMAT_JPEG);
            return NO_ERROR;
        }
    ...
    status_t QCameraHardwareInterface::setFullLiveshot()
    {
        ...
        if(value != 0)
        {
            ...
            //VFE2x secondary port doesnot support FWVGA+ output
            if((mDimension.video_width > 864) || (mDimension.video_height > 480))
            {
                ALOGE("%s video size is larger than FWVGA, disable full size livesnapshot\r\n", __func__);
                //mParameters.setPictureSize(DEFAULT_PICTURE_WIDTH, DEFAULT_PICTURE_HEIGHT);
                //mDimension.picture_width = DEFAULT_PICTURE_WIDTH;
                //mDimension.picture_height = DEFAULT_PICTURE_HEIGHT;
                value = 0;
    ...
    status_t QCameraHardwareInterface::setPictureSizeTable(void)
    {
        ...
        ALOGD("%s: Max Picture Sizes Supported: %d X %d", __func__,
              dim.width, dim.height);
        mSensorFakeWidth = dim.camera_fake_width;
        mSensorFakeHeight = dim.camera_fake_height;
        mActualSensorWidth		= dim.width;
        mActualSensorHeight = dim.height;
        ALOGD("%s: Max Picture Sizes Expectted By User: %d X %d", __func__,
              mSensorFakeWidth, mSensorFakeHeight);
        if((mSensorFakeWidth > mActualSensorWidth) || (mSensorFakeHeight > mActualSensorHeight))
        {
            mCameraResolutionUpSupport = true ;
            if(mCameraResolutionUpSupport)
            {
                tmpWidth  = dim.width ;
                tmpHeight = dim.height ;
                dim.width  = mSensorFakeWidth ;
                dim.height = mSensorFakeHeight ;
            }
        }
