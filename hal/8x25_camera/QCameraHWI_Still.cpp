...
//#define DUMP_JPEG_INPUT 0
//#define DUMP_JPEG_INPUT1 0
...
namespace android
{
    ...
    initSnapshotBuffers(cam_ctrl_dimension_t *dim, int num_of_buf)
    {
        ...
        if(!mHalCamCtrl->mCaptureScalingUp)
            ...
            frame_len = PAD_TO_4K (CEILING16(ui_set_picture_width) * CEILING16(ui_set_picture_height) * 3 / 2);
        //***************UpScaling End*********************
        ...
        if(!isLiveSnapshot())
        {
            ...
            if(!mHalCamCtrl->mCaptureScalingUp)
                ...
                frame_len = PAD_TO_4K(PAD_TO_4K (CEILING16(ui_set_picture_width) * CEILING16(ui_set_picture_height) * 3 / 2) * 2);
            //***************UpScaling End*********************
               ...
               bool enqueued)
    {
        ...
        //***************UpScaling Start*********************
        uint8_t *tmp1_buf = NULL, *tmp2_buf = NULL;
        uint32_t picture_width, picture_height, thumb_width, thumb_height;
        uint32_t strtch_size;
        uint8_t  mZooming = false;
        uint32_t y_buf_offset;
        uint32_t cbcr_buf_offset ;
        uint32_t buf_size ;
        uint8_t  num_planes;
        uint32_t planes[10];
        uint32_t sensor_width, sensor_height;
        ...
        else      /*not busy and new buffer (first job)*/
        {
            ...
            LOGD("%s: dimension.orig_picture_dx = %d, dimension.orig_picture_dy =%d", __func__, dimension.orig_picture_dx, dimension.orig_picture_dy);
            ...
            LOGD("%s: dimension.thumbnail_width = %d, dimension.thumbnail_height =%d", __func__, dimension.thumbnail_width, dimension.thumbnail_height);
            ...
            LOGD("%s: crop.in2 resolution: %dx%d out2 resolution: %dx%d", __func__,
                 crop.in2_w, crop.in2_h,
                 crop.out2_w, crop.out2_h);

            LOGD("%s: main_crop_offset.x = %d, main_crop_offset.y = %d", __func__,
                 main_crop_offset.x,
                 main_crop_offset.y);
            ...
            if(!mZooming)
            {
                if(mHalCamCtrl->mCaptureScalingUp)
                {
                    LOGE("%s: Doing the image Scaling up.......", __func__);
                    strtch_size = PAD_TO_4K(CEILING16(ui_set_picture_width) * CEILING16(ui_set_picture_height) * 3 / 2);

                    imgFrame.list.count = 1;

                    blitReq = &imgFrame.list.req[0];

                    blitReq->src.width  = sensor_width;
                    blitReq->src.height = sensor_height;
                    blitReq->src.format = MDP_Y_CRCB_H2V2;
                    blitReq->src.offset = mainframe->phy_offset;
                    blitReq->src.memory_id = mainframe->fd;

                    LOGD("%s: mainframe->fd = %d", __func__, mainframe->fd);
                    LOGD("%s: mainframe->phy_offset = %d", __func__, mainframe->phy_offset);

                    blitReq->dst.width  = ui_set_picture_width;
                    blitReq->dst.height = ui_set_picture_height;
                    blitReq->dst.format = MDP_Y_CRCB_H2V2;
                    blitReq->dst.offset = mainframe->phy_offset + (uint32_t)(strtch_size);
                    blitReq->dst.memory_id = mainframe->fd;
                    ...
                    blitReq->dst_rect.w = ui_set_picture_width;
                    blitReq->dst_rect.h = ui_set_picture_height;

                    blitReq->src_rect.x = 0;
                    blitReq->src_rect.y = 0;
                    blitReq->src_rect.w = sensor_width;
                    blitReq->src_rect.h = sensor_height;

                    LOGD("%s: mainframe->buffer = %ld", __func__, mainframe->buffer);
                    LOGD("%s: mainframe->y_off = %d", __func__, mainframe->y_off);
                    LOGD("%s: mainframe->cbcr_off = %d", __func__, mainframe->cbcr_off);


                    if (fb_fd == -1)
                    {
                        LOGE("%s: Open fb0!", __func__);
                        fb_fd = open("/dev/graphics/fb0", O_RDWR, 0);
                        if (fb_fd < 0)
                        {
                            LOGE("%s: Open fb0 failed!", __func__);
                    ...
                    LOGD("%s : memcpy start!", __func__);

                    mm_jpeg_encoder_get_buffer_offset1(ui_set_picture_width, ui_set_picture_height,
                                                       &y_buf_offset, &cbcr_buf_offset, &buf_size);

                    LOGD("%s : buffer offset = %d", __func__, y_buf_offset);


                    tmp1_buf = (uint8_t *)(mainframe->buffer + y_buf_offset);
                    tmp2_buf = (uint8_t *)(mainframe->buffer + strtch_size);
                    memcpy(tmp1_buf, tmp2_buf, ui_set_picture_width * ui_set_picture_height);

                    tmp1_buf = (uint8_t *)(mainframe->buffer + cbcr_buf_offset);
                    tmp2_buf = (uint8_t *)(mainframe->buffer + strtch_size + ui_set_picture_width * ui_set_picture_height);
                    memcpy(tmp1_buf, tmp2_buf, ui_set_picture_width * ui_set_picture_height >> 1);



                    /*
                    			    blitReq->src.width  = ui_set_picture_width;
                    			    blitReq->src.height = ui_set_picture_height;
                    			    blitReq->src.format = MDP_Y_CRCB_H2V2;
                    			    blitReq->src.offset = mainframe->phy_offset+(uint32_t)(strtch_size);
                    			    blitReq->src.memory_id = mainframe->fd;

                    			    blitReq->dst.width  = ui_set_picture_width;
                    			    blitReq->dst.height = ui_set_picture_height;
                    			    blitReq->dst.format = MDP_Y_CRCB_H2V2;
                    			    blitReq->dst.offset = mainframe->phy_offset;
                    			    blitReq->dst.memory_id = mainframe->fd;
                    			    blitReq->transp_mask = 0xffffffff;
                    			    blitReq->flags = 0;
                    			    blitReq->alpha  = 0xFF;
                    			    ...
                    			    blitReq->src_rect.x = 0;
                    			    blitReq->src_rect.y = 0;
                    			    blitReq->src_rect.w = ui_set_picture_width;
                    			    blitReq->src_rect.h = ui_set_picture_height;

                    			    LOGE("%s: mainframe->buffer = %ld",__func__, mainframe->buffer);
                    				LOGE("%s: mainframe->y_off = %d", __func__,mainframe->y_off);
                    				LOGE("%s: mainframe->cbcr_off = %d", __func__,mainframe->cbcr_off);
