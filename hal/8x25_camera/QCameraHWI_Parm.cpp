...
#define DONT_CARE AF_MODE_MAX
    ...
    void QCameraHardwareInterface::initDefaultParameters()
    {
        ...
        mParameters.set(CameraParameters:: KEY_PANORAMA_FPS, "false");
        ...
        ////set default value for Focus Distance
        mFocusDistance = "Infinity,Infinity,Infinity";
        ...
        //8x25 do not support power modes.
        ...
        //Set HDR
        rc = cam_config_is_parm_supported(mCameraId, MM_CAMERA_PARM_HDR);
        if(!rc)
        {
            LOGE("%s: HDR is  not supported", __func__);
    ...
    */
    ...
    status_t QCameraHardwareInterface::updateFocusDistances()
    {
                               ...
                               &focusDistances) == MM_CAMERA_OK)
        {
            ...
            //focus distance value check.
            if((focusDistances.focus_distance[0] < 0)
                    || (focusDistances.focus_distance[1] < 0)
                    || (focusDistances.focus_distance[2] < 0)
                    || (focusDistances.focus_distance[0] > focusDistances.focus_distance[1])
                    || (focusDistances.focus_distance[1] > focusDistances.focus_distance[2])
              )
            {
                LOGE("%s: KEY_FOCUS_DISTANCES check failure, using Infinity instead", __FUNCTION__);
                ...
                snprintf(buffer, sizeof(buffer), "Infinity");
                str.append(buffer);
                LOGE("%s: setting KEY_FOCUS_DISTANCES data invalid, set to infinity", __FUNCTION__);
            }
            //set all distances to infinity if focus mode is infinity
            else if(mFocusMode == AF_MODE_INFINITY)
            {
                snprintf(buffer, sizeof(buffer), "Infinity,");
                str.append(buffer);
                snprintf(buffer, sizeof(buffer), "Infinity,");
        ...
        if (str != NULL)
        {
            ...
            if (value != NOT_FOUND)
            {
                ...
                else
                {
                    ...
                    cam_config_get_parm(mCameraId, MM_CAMERA_PARM_BESTSHOT_RECONFIGURE,
                                        &bestshot_reconfigure);
                    if(bestshot_reconfigure && !mBestShotMode)
                    {
                        if (mEffects != value)
                        {
                            mEffects = value;
    ...
    status_t QCameraHardwareInterface::setPreviewFrameRate(const CameraParameters &params)
    {
        ...
        const char *str = params.get(CameraParameters::KEY_PANORAMA_FPS);
        if(str != NULL)
        {
            if(strcmp(str, "true") == 0)
            {
                int mNuberOfVFEOutputs;
                bool ret = cam_config_get_parm(mCameraId,
    ...
    status_t QCameraHardwareInterface::setPictureSize(const CameraParameters &params)
    {
        ...
        for (int i = 0; i < mSupportedPictureSizesCount; ++i)
        {
                    ...
                    && height == mPictureSizesPtr[i].height)
            {
                ...
                //***************UpScaling Start*********************
                if(myMode & CAMERA_ZSL_MODE)
                    mCameraResolutionUpSupport = false ;

                if(mCameraResolutionUpSupport)
                {
                    mUISetCameraWidth = width ;
                    mUISetCameraHeight = height;

                    LOGD("Max Sensor output size = : %dx%d", mActualSensorWidth, mActualSensorHeight);
                    LOGD("UI setting output size = : %dx%d", mUISetCameraWidth, mUISetCameraHeight);

                    if(width > mActualSensorWidth || height > mActualSensorHeight)
                    {
                        mCaptureScalingUp = true;
                        mDimension.picture_width = mActualSensorWidth;
                        mDimension.picture_height = mActualSensorHeight;
                        LOGD("UI setting size is great than sensor output, CaptureScalingUp = : %d", mCaptureScalingUp);
                    }
                    else
                    {
                        mCaptureScalingUp = false;
                        ...
                        LOGD("UI setting size is less than sensor output, CaptureScalingUp = : %d", mCaptureScalingUp);
                    }
                    LOGD("mDimension picture width x height = : %dx%d", mDimension.picture_width, mDimension.picture_height);
                ...
                else
                {
                    if(width > mActualSensorWidth || height > mActualSensorHeight)
                    {
                        mUISetCameraWidth = mActualSensorWidth ;
                        mUISetCameraHeight = mActualSensorHeight;
                        mDimension.picture_width  = mActualSensorWidth;
                        mDimension.picture_height = mActualSensorHeight;
                    }
                    else
    ...
    */
    bool QCameraHardwareInterface::supportVideoSizeOfHFR(const CameraParameters &params)
    {
        int width;
        int height;
        const char *str = params.get(CameraParameters::KEY_VIDEO_SIZE);
        if (str != NULL)
        {
    ...
    status_t QCameraHardwareInterface::setPictureSizeTable(void)
    {
        ...
        LOGD("%s: Max Picture Sizes Expectted By User: %d X %d", __func__,
        ...
        if(mCameraResolutionUpSupport)
        {
            LOGE("%s: Exceptted sensor size great than actual sensor output size, need doing Scaling up.",
                 __func__, dim.width, dim.height);
