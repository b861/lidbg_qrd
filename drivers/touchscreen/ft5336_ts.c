...
static irqreturn_t ft5x06_ts_interrupt(int irq, void *dev_id)
{
    ...
    //	print_hex_dump_bytes("ft5336 i2c data:", DUMP_PREFIX_NONE, buf, data->tch_data_len);

    //Key detect by Victor Wen
#define KEY_X_POS 3
#define KEY_Y_POS 5
    key_y = 0;
    key_y = buf[KEY_Y_POS + 1] | (buf[KEY_Y_POS] << 8);
    //	pr_info("ft5336:key_y=%d\n", key_y);
    if (unlikely(key_y == 1350))
    {
        //		pr_info("ft5336:key detect\n");
        key_x = 0;
        key_x = buf[KEY_X_POS + 1] | (buf[KEY_X_POS] << 8);

        //ignore key contact -- key keep in down status
        if (key_x & 0x8000)
            return IRQ_HANDLED;

        key_down = !(key_x & 0x4000);

        if ((key_x & 0xFFF) == 160)
        {
            input_report_key(ip_dev, KEY_MENU, key_down);
        }
        else if ((key_x & 0xFFF) == 600)
        {
            input_report_key(ip_dev, KEY_BACK, key_down);
        }
        else
        {
            pr_info("ft5336:fatal unkown key");
        }
