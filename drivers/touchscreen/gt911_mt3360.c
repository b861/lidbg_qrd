...
static s32 gtp_init_panel(struct goodix_ts_data *ts, char *ic_type, char *Firmware_version)
{
    ...
    ret = gtp_i2c_read(ts->client, configed_data, 186);
    for(i = 0; i < 188; i++)
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_mt_init_slots(ts->input_dev, 15);
#else
...
static int init_cdev_ts(void)
{
    ...
    lidbg("gt911===========alloc_chrdev_region result:%d , major_number_ts:%d\n", result, major_number_ts);
...
static int __devinit goodix_ts_init(void)
{
    ...
    SOC_IO_Output(0, 27, 1);
    msleep(200);
    SOC_IO_Output(0, 27, 0);//NOTE:GT911 SHUTDOWN PIN ,set hight to work.
    msleep(300);
#else
    SOC_IO_Output(0, 27, 0);
    msleep(200);
    SOC_IO_Output(0, 27, 1);//NOTE:GT911 SHUTDOWN PIN ,set hight to work.
    msleep(300);
#endif
#endif
    s32 ret;
