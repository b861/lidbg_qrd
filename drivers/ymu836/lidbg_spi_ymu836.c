...
struct spi_device *ymu836_spi=NULL;
...
S32	YAD1DD_Read(U08 u08Page,U08 u08Adr,U08 *pu08Data,U32 u32Size)
{
	...
	if (( pu08Data	!= NULL )&& ( u32Size	!= 0 ))
	{
		...
		if (( s32Ret	== YAD1DD_SUCCESS )&& ( u08Com1	!= YAD1DD_SPI_COM_READ ))
		{
 ...
 *	get_variable_size
 ******************************************************************************/
static S32	get_variable_size(const U08 *pu08Script,U32 u32Size,U32 * pu32Value)
{
 ...
 *	wait_flag
 ******************************************************************************/
static S32	wait_flag(U08 u08Page,const U08 *pu08Script)
{
	...
	u08Reg	= 0;
	s32Ret	= YAD1DD_SUCCESS;
	//printk("u08Adr=0x%x,u08Mask=0x%x,u08Val=0x%x,u08TO=0x%x\n",u08Adr,u08Mask,u08Val,u08TO);
 ...
 *	mem_ctrl
 ******************************************************************************/
static S32	mem_ctrl(const U08 *pu08Script)
{
	U08		u08Ope1, u08Ope2;
	U08 *	pu08Mem;
	S32		s32Ret;
 ...
 *	run_1_command
 ******************************************************************************/
static S32	run_1_command(const U08 *pu08Script,U32	u32Size)
{
	S32					s32Ret;
...
int yum836_send_config(void *data)
{
	S32	s32Ret;
	msleep(100);
	s32Ret=YAD1DD_BatchProcess(au08testpath01_00,ARRAY_SIZE(au08testpath01_00));
	if(s32Ret)
		printk("yum836_send_config au08testpath_flyaudio_01_00 fail!\n");
	else
		printk("yum836_send_config au08testpath_flyaudio_01_00 ok!\n");

	s32Ret=YAD1DD_BatchProcess(au08testpath01_16,ARRAY_SIZE(au08testpath01_16));
	if(s32Ret)
		printk("yum836_send_config au08testpath_flyaudio_01_16 fail!\n");
	else
		printk("yum836_send_config au08testpath_flyaudio_01_16 ok!\n");
	
	s32Ret=YAD1DD_BatchProcess(au08testpath01_17,ARRAY_SIZE(au08testpath01_17));
	if(s32Ret)
		printk("yum836_send_config au08testpath_flyaudio_01_17 fail!\n");
	else
		printk("yum836_send_config au08testpath_flyaudio_01_17 ok!\n");
#if 0
	s32Ret=YAD1DD_BatchProcess(au08testpath01_18,ARRAY_SIZE(au08testpath01_18));
	if(s32Ret)
		printk("yum836_send_config err!\n");
	else
		printk("yum836_send_config au08testpath_flyaudio_01_18 ok!\n");
#endif
	return s32Ret;
}
...
static int ymu836_spi_probe(struct spi_device *spi)
{
	...
	ymu836_spi = spi;
	CREATE_KTHREAD(yum836_send_config,NULL);
	return 0;
}

static int ymu836_spi_remove(struct spi_device *spi)
{
	return 0;
}

static const struct spi_device_id ymu836_id[] = {
	{ "ymu836", 0 },
	{ }
};
MODULE_DEVICE_TABLE(spi, ymu836_id);

static struct spi_driver ymu836_driver = {
	.driver = {
		.owner  = THIS_MODULE,
		.name	= "ymu836"
	},
...
static int __init yum836_init(void)
{
	int status;
	...
	DUMP_BUILD_TIME;
    LIDBG_GET;
	status = spi_register_driver(&ymu836_driver);
	if (status < 0) {
		pr_err("%s: spi_register_driver ymu836_spi_driver failure. status = %d\n", __func__, status);
	}
	pr_err("%s: spi_register_driver ymu836_spi_driver success. status = %d\n", __func__, status);
	master = spi_busnum_to_master(YMU836_SPI_BUS);
    if (!master)
    {
        status = -ENODEV;
        goto error_busnum;
...
error_mem:
error_busnum:
    spi_unregister_driver(&ymu836_driver);
    return status;
}

static void __exit yum836_exit(void)
{
	if(ymu836_spi!=NULL)
	spi_unregister_device(ymu836_spi);
	spi_unregister_driver(&ymu836_driver);
}


module_init(yum836_init);
module_exit(yum836_exit);
MODULE_DESCRIPTION("YUM driver");
MODULE_LICENSE("GPL");
