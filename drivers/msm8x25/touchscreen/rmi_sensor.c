...
void attn_notify(struct rmi_sensor_driver *sensor)
{
    ...
    list_for_each_entry(functionInfo, &sensor->functions, link)
    {
        if (functionInfo->function_device && functionInfo->function_device->rmi_funcs->attention)
...
int rmi_sensor_register_functions(struct rmi_sensor_driver *sensor)
{
    ...
    pr_debug("[futengfei]%s: ---------------Registering sensor functions\n", __func__);
                    ...
                    if (rmi_fd.interruptSrcCnt)
                    {
                        ...
                        if (fn)
                        {
                            ...
                            */
                            ...
                            if (retval)
                            {
                                printk(KERN_ERR "%s:  errerrerrerrerr---Failed rmi_function_register_device.\n",
                                       __func__);
                                return retval;
                            }
                        }
...
int rmi_sensor_register_driver(struct rmi_sensor_driver *driver)
{
    ...
    if (ret)
    {
        printk( "[futengfei] %s: Failed rmi_sensor_register_functions %d\n",			__func__, ret);
    }
