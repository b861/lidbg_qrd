...
static void f11_set_abs_params(struct rmi_function_device *function_device)
{
    ...
    input_set_abs_params(function_device->input, ABS_MT_POSITION_X, xMin, xMax,	0, 0);
    input_set_abs_params(function_device->input, ABS_MT_POSITION_Y, yMin, yMax,	0, 0);
    input_set_abs_params(function_device->input, ABS_MT_PRESSURE, 0, 15, 0, 0);
    //dd

    printk(KERN_DEBUG "%s: input_set_abs_paramsSet ranges X=[%d..%d] Y=[%d..%d].", __func__, xMin, xMax, yMin, yMax);
#endif
}
...
int FN_11_init(struct rmi_function_device *function_device)
{
    ...
    __set_bit(EV_KEY, function_device->input->evbit);
    __set_bit(EV_ABS, function_device->input->evbit);
    __set_bit(BTN_TOUCH, function_device->input->evbit);
    __set_bit(EV_SYN, function_device->input->evbit);
