...
#define ACCEL_USE_INT2 		0

#if		ACCEL_INT_EN
#define CONFIG_BMA_ENABLE_NEWDATA_INT
#endif
		...
		unsigned char reg_addr, unsigned char *data)
{
	...
	dummy = SOC_I2C_Rec(ACCEL_I2C_BUS, client->addr, reg_addr, data, 1);
	if (dummy < 0)
		return -EIO;
		...
		unsigned char reg_addr, unsigned char *data)
{
	...
	buf[0] = reg_addr;
	buf[1] = *data;
	dummy = SOC_I2C_Send(ACCEL_I2C_BUS, client->addr, buf, 2);
		...
		unsigned char reg_addr, unsigned char *data, unsigned char len)
{
	...
	dummy = SOC_I2C_Rec(ACCEL_I2C_BUS, client->addr, reg_addr, data, len);
	if (dummy < 0)
		return -EIO;
	return 0;
}
		...
		u8 *data, u16 len)
{
	...
	for (retry = 0; retry < BMA_MAX_RETRY_I2C_XFER; retry++) {
		if (SOC_I2C_Rec(ACCEL_I2C_BUS, client->addr, reg_addr, data, len) > 0)
			break;
		else
			I2C_RETRY_DELAY();
	}
		...
		const struct i2c_device_id *id)
{
	...
	pdata->int_en = ACCEL_INT_EN;
	pdata->place = ACCEL_PLACE;
	pdata->poll_interval = ACCEL_INIT_INTERVAL;
	pdata->use_int2 = ACCEL_USE_INT2;
	pdata->gpio_int1 = ACCEL_INT1 + GPIO_MAP_OFFSET;
	...
	input_set_drvdata(dev_acc, data);

	err = input_register_device(dev_acc);
	if (err < 0) {
		dev_err(&client->dev,
...
remove_bst_acc_sysfs_exit:
	sysfs_remove_group(&data->bst_acc->dev.kobj,
			&bma2x2_attribute_group);
bst_free_exit:
	input_unregister_device(dev_acc);

bst_free_acc_exit:
	input_free_device(dev_acc);
