...
int jpeg_init_decoder(int width, int height)
{
	...
	jpeg_ctx = (jpeg_decoder_context_t *)calloc(1, sizeof(jpeg_decoder_context_t));
	if(jpeg_ctx == NULL)
	{
		LIDBG_PRINT("V4L2_CORE: FATAL memory allocation failure (jpeg_init_decoder): %s\n", strerror(errno));
		exit(-1);
	}
	...
	jpeg_ctx->tmp_frame = (uint8_t *)calloc(jpeg_ctx->pic_size, sizeof(uint8_t));
...
int jpeg_decode(uint8_t *in_buf, uint8_t *out_buf, int size)
{
	/*asserts*/
	assert(in_buf != NULL);
	assert(out_buf != NULL);
	...
	int err = 0;
	int isInitHuffman = 0;
	decdata = (jpeg_decdata *)calloc(1, sizeof(struct jpeg_decdata));
	if(decdata == NULL)
	{
		LIDBG_PRINT("V4L2_CORE: FATAL memory allocation failure (jpeg_decode): %s\n", strerror(errno));
		exit(-1);
	}
	...
	//LIDBG_PRINT("V4L2_CORE: intheight= %d intwidth = %d\n", intheight, intwidth);
	...
	if (!info.ns)
	{
		LIDBG_PRINT("V4L2_CORE: (jpeg decoder) info ns %d/n",info.ns);
		err = E_NOT_YCBCR_ERR;
		goto error;
	}
	/*for each scan*/
	...
	if (i != 0 || j != 63 || m != 0)
	{
		LIDBG_PRINT("V4L2_CORE: (jpeg decoder) FW error,not seq DCT ??\n");
	}
	...
	*/
	...
	//	if(*pic == NULL)
	//	{
	//		fLIDBG_PRINT(stderr, "V4L2_CORE: FATAL memory allocation failure (alloc_v4l2_frames): %s\n", strerror(errno));
	//		exit(-1);
	//	}
	//}
