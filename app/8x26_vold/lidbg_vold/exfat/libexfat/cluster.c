 ...
 */
static s64 s2o(const struct exfat* ef, s64 sector)
{
 ...
 * Cluster to sector.
 */
static s64 c2s(const struct exfat* ef, cluster_t cluster)
{
	if (cluster < EXFAT_FIRST_DATA_CLUSTER)
		exfat_bug("invalid cluster number %u", cluster);
	return le32_to_cpu(ef->sb->cluster_sector_start) +
		((s64) (cluster - EXFAT_FIRST_DATA_CLUSTER) << ef->sb->spc_bits);
}
 ...
 * Sector to cluster.
 */
static cluster_t s2c(const struct exfat* ef, s64 sector)
{
...
static void erase_raw(struct exfat* ef, size_t size, s64 offset)
{
