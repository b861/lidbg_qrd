...
private:
    ...
    int                    launch_device(struct usb_device *dev, int level);
    bool                   search(const char *buf, const char *key, char *value, int len);
    int                    doUsbModeSwitch(NetlinkEvent *evt);
...
public:
	...
	void handleUsbModeSwitchEvent(NetlinkEvent *evt);
