...
bool VolumeManager::search(const char *buf, const char *key, char *value, int len)
{
    bool ret=false;
    char *p, *str;
    int  i;
...
int VolumeManager::launch_device(struct usb_device *dev, int level)
{
    #define CONTINUE_LOOP(ret)    if((!ret)){ continue;}  
    usb_dev_handle *udev;
    char cmd[1024];
    FILE *file;
    char buf[256];
    char idvendor[32], idproduct[32];
    char vendorid[32], productid[32], magicstr[256];
    char atportname[32],modemportname[32],devicename[64],type[32];
    bool found = false, ret = false;

    snprintf(idvendor, sizeof(idvendor), "%04X", dev->descriptor.idVendor);
    snprintf(idproduct, sizeof(idproduct), "%04X", dev->descriptor.idProduct);
    LOGD("idvendor:%s, idproduct:%s\n", idvendor, idproduct);
    file = fopen ("/system/etc/usbdongles.conf", "r");
    if (file) {
        while (fgets(buf, 256, file)) {
            memset (vendorid, 0, sizeof(vendorid));
            ret = search (buf, "VendorID", vendorid, sizeof(vendorid));
            CONTINUE_LOOP(ret);
            LOGD("cmp vendor, idvendor:%s, vendorid:%s\n", idvendor, vendorid);
            if (strcmp (idvendor, vendorid))
                continue;

            memset (productid, 0, sizeof(productid));
            ret = search (buf, "ProductID", productid, sizeof(productid));
            CONTINUE_LOOP(ret);
            LOGD("cmp product ,idproduct:%s, productid:%s\n", idproduct, productid);
            if (strcmp (idproduct, productid))
                continue;


            memset (magicstr, 0, sizeof(magicstr));
            ret = search (buf, "MagicString", magicstr, sizeof(magicstr));
            CONTINUE_LOOP(ret);

            //LOGD("magicstr:%s\n", magicstr);

            memset (atportname, 0, sizeof(atportname));
            ret = search (buf, "ATPort", atportname, sizeof(atportname));
            CONTINUE_LOOP(ret);
            LOGD("at_port_name:%s\n", atportname);

            memset (modemportname, 0, sizeof(modemportname));
            ret = search (buf, "ModemPort", modemportname, sizeof(modemportname));
            CONTINUE_LOOP(ret);
            LOGD("modem_port_name:%s\n", modemportname);

            memset (devicename, 0, sizeof(devicename));
            ret = search (buf, "Device", devicename, sizeof(devicename));
            CONTINUE_LOOP(ret);
            LOGD("device_name:%s\n", devicename);

            memset (type, 0, sizeof(type));
            ret = search (buf, "Type", type, sizeof(type));
            CONTINUE_LOOP(ret);
            LOGD("type:%s\n", type);
        ...
        if (ret && found) {
			...
			sprintf (cmd, "usb_modeswitch -v %s -p %s -M %s ", idvendor, productid, magicstr);
            LOGD ("[cmd:%s]\n", cmd);
            ret = system (cmd);
            LOGD ("return code is:%d\n", ret);
            ...
            LOGI("failed since keyword script file @ //system//etc//usbdongles.conf!\n");
		}
		
        fclose(file);
    }
    else {
        LOGI("failed to open file @ //system//etc//usbdongles.conf!\n");
    }
    return ret;
}


int VolumeManager::doUsbModeSwitch(NetlinkEvent *evt)
{
    char i = 0;
    char *param = NULL;
    char *cmd = NULL;
    ...
    param = (char *)(evt->findParam((const char *)"DEVTYPE"));
    if (param == NULL) {
		LOGD("No DEVTYPE\n");
		return 1;
    }
    LOGD("devtype:%s\n", param);
    strcpy(devtype, param);

    if((action == 1) && !strcmp(devtype, "usb_device")) {
		param = NULL;
		param = (char *)evt->findParam((const char *)"PRODUCT");
		if (param == NULL) {
			LOGD("No PRODUCT\n");
			return 1;
		}
		...
		if(!file) {
			...
			asprintf(&cmd, "usb_modeswitch -W -I -v %s -p %s -c %s &",usb_vid, usb_pid, fbuf);
			//asprintf(&cmd, "usb_modeswitch -v %s -p %s -M %s ",usb_vid, usb_pid, "555342430038AA852400000080000685000000240000000000000000000000");
			//asprintf(&cmd, "usb_modeswitch -I -W -D -v 0x%s -p 0x%s -u 2",usb_vid, usb_pid);  
    ...
    } else if ((action == 2) && !strcmp(devtype, "usb_device")) {
		LOGI("remove 3g dongle !!!\n");
		property_set("net.3gdongle.exist", "false");
		property_set("net.3gdongle.devicename", "");
		property_set("net.3gdongle.portname.at", "");
		property_set("net.3gdongle.portname.modem", "");
		property_set("net.3gdongle.type", "");
    } else {
		LOGI("Not usefull devicetype !!!\n"); 
    }

    return 0;
}

void VolumeManager::handleUsbModeSwitchEvent(NetlinkEvent *evt)
{
	struct usb_bus *bus;
	//const char *devtype = evt->findParam("DEVTYPE");
	if(1 != evt->getAction()) //Action Add
	{
		return;
	}
	...
	/* For debug 3G Dongle */	
	//evt->dump();
	//SLOGW("============== handleUsbModeSwitchEvent: getSubsystem: %s ===================", evt->getSubsystem());
	//SLOGW("============== handleUsbModeSwitchEvent: getPath: %s ===================", evt->getPath());
	//SLOGW("============== handleUsbModeSwitchEvent: getAction: %d ===================", evt->getAction());
...
int VolumeManager::mountVolume(const char *label) {
    ...
    if (!v) {
        errno = ENOENT;
		SLOGE("Max Xia: Error lookupVolume (%s)", strerror(errno));
        return -1;
    }

    rc=v->mountVol();
    if(rc!=0){
	SLOGE("mount err=%d,remount volume",rc);
	rc=v->mountVol();
    }
...
int VolumeManager::unmountAllAsecsInDir(const char *directory) {
    ...
    struct dirent *dent = (struct dirent *) malloc(sizeof(struct dirent));
    if (dent == NULL) {
