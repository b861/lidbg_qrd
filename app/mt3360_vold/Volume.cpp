...
void Volume::setReserveMedia(int flags)
{
    mDiskReserve = flags ? true : false;
}
...
bool Volume::isMountpointMounted(const char *path) {
    ...
    while(fgets(line, sizeof(line), fp)) {
        ...
        if (!strcmp(mount_path, path)) {
			SLOGE("Max Xia: isMountpointMounted: (%s)(%s)(%s)", device, mount_path, rest);
            fclose(fp);
	    return false;
        }
...
int Volume::mountVol() {
    ...
    memset(deviceNodes, 0, sizeof(deviceNodes));
    ...
    n = getDeviceNodes((dev_t *) &deviceNodes, 32);
    if (!n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        fsType = Vfs::getFileSysType(devicePath);
        if (-1 == fsType)	{
            errno = EIO;
            SLOGE("%s failed getFileSysType (%s)", devicePath, strerror(errno));
            setState(Volume::State_Idle);
            return -1;
        }
        ...
        if (mDiskReserve) {
	          ...
	          */
	          ...
	          if (Vfs::doMount(devicePath, "/mnt/secure/staging", fsType, false, false, false,
                AID_SYSTEM, gid, 0702, true)) {
	              SLOGE("%s failed to mount via VFAT/NTFS/EXFAT (%s)\n", devicePath, strerror(errno));
                continue;
	          }
            ...
            if (Vfs::doMount(devicePath, getMountpoint(), fsType, false, false, false,
                1000, 1015, 0702, false)){
                    SLOGE("%s failed to mount via VFAT/NTFS/EXFAT (%s)\n", devicePath, strerror(errno));
                    continue;
            } else {
                    SLOGD("-------------------- Mount %s Success ------------------\n", devicePath);
            }
        }
        //setState(Volume::State_Mounted);
        setState(Volume::State_Mounted);
...
int Volume::unmountVol(bool force, bool revert) {
    ...
    if (doUnmount(getMountpoint(), force)) {
        ...
        if (doUnmount(getMountpoint(), force)){
            SLOGE("Failed to umount %s => %s (%s)", getMountpoint(), strerror(errno));            
            goto fail_recreate_bindmount;            
        }
    }
