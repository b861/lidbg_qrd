...
/*#if 1//GUITAR_CONFIG_43
	uint8_t config_info[54]={0x30,	0x19,0x05,0x06,0x28,0x02,0x14,0x14,0x10,0x50,0xB8,TOUCH_MAX_WIDTH>>8,TOUCH_MAX_WIDTH&0xFF,
...
//printk("===========come intogoodix_ts_work_func=======\n");
...
//printk("======read coordinate finger=%d, [X=%d, Y=%d], pressure=%d ====\n", \
//			finger_list.pointer[count].num,finger_list.pointer[count].x,finger_list.pointer[count].y,finger_list.pointer[count].pressure);
...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
	...
	queue_work(goodix_wq, &ts->work);
	//printk("========end  goodix_ts_irq_handler=========\n");
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//printk("=======WANG after SHUTDOWN_PORT ========\n");


	i2c_connect_client = client;				//used by Guitar Updating.
	msleep(16);
	//add a buf and set the address, by wangyihong
	unsigned int buf = 0;
	client->addr = 0x55;        
	for(retry=0; retry < 5; retry++)
	{
		//printk("=======WANG beforerite_bytes ========\n");
	
		ret =i2c_write_bytes(client, &buf, 1);	//Test i2c.
	...
	//printk("=======WANG afterinput_allocate_device ========\n");
...
//#ifndef GOODIX_MULTI_TOUCH	
	...
	//printk("=======WANG input_register_device success ========\n");
	...
	if (client->irq)
	{
		...
		//printk("=======WANG gpio_request success ========\n");

		ret = gpio_tlmm_config(GPIO_CFG(INT_PORT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,  GPIO_CFG_16MA), GPIO_CFG_ENABLE);
		...
		{	
			...
			//printk("=======WANG request_irq success ========\n");
		}
	}
#endif
