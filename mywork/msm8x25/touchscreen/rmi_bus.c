...
int rmi_register_sensor(struct rmi_phys_driver *rpd, struct rmi_sensordata *sensordata)
{
    ...
    */
    if (!rpd->name)
    {
        printk("[futengfei]%s: Physical driver must specify a name",			__func__);
        return -EINVAL;
    }
    if (!rpd->write)
    {
        printk("[futengfei]%s: Physical driver %s must specify a writer.",		__func__, rpd->name);
        return -EINVAL;
    }
    if (!rpd->read)
    {
        printk("[futengfei]%s: Physical driver %s must specify a reader.",			__func__, rpd->name);
        return -EINVAL;
    }
    if (!rpd->write_multiple)
    {
        printk( "[futengfei]%s: Physical driver %s must specify a multiple writer.",			__func__, rpd->name);
        return -EINVAL;
    }
    if (!rpd->read_multiple)
    {
        printk( "[futengfei]%s: Physical driver %s must specify a multiple reader.",			__func__, rpd->name);
        return -EINVAL;
    }

    /* Get some information from the device */
    printk( "[futengfei] %s:-------------> Identifying sensors by presence of F01\n", __func__);
     ...
     */
    for (i = PDT_START_SCAN_LOCATION;	/* Register the rmi sensor driver */	i >= PDT_END_SCAN_LOCATION;		i -= PDT_ENTRY_SIZE)
    {
        retval = rpd->read_multiple(rpd, i, (char *)&rmi_fd, sizeof(rmi_fd));
        printk("[futengfei] rmi_register_sensor------retval=[%d]---->1\n", retval);
        if (!retval)
        {
            rfi = NULL;
            printk("[futengfei] rmi_register_sensor----rmi_fd.functionNum=[0x%x]------->2\n", rmi_fd.functionNum);
            ...
            if (rmi_fd.functionNum != 0x00 && rmi_fd.functionNum != 0xff)
            {
                ...
                {
                    printk( "[]%s: F01 Found - RMI Device Control", __func__);
                    printk("[futengfei] rmi_register_sensor-------[%x]---->3\n", (rmi_fd.functionNum & 0xff));
                    ...
                    * device and sensor driver for it. */
                    ...
                    if (!rmi_sensor_dev)
                    {
                        printk( "[futengfei] %s: Error allocating memory for rmi_sensor_device G\n", __func__);
                        retval = -ENOMEM;
                        goto exit_fail;
                    }
                    ...
                    if (retval < 0)
                    {
                        printk( "[futengfei] %s: Error %d registering sensor device.\n", __func__, retval);
                        goto exit_fail;
                    }
                    ...
                    {
                        printk( "[futengfei] %s: Error allocating memory for rmi_sensor_driver G\n", __func__);
                        retval = -ENOMEM;
                        goto exit_fail;
                    }
                    ...
                    if (retval < 0)
                    {
                        printk( "[futengfei] %s: Error %d registering sensor driver. G\n", __func__, retval);
                        goto exit_fail;
                    }
                ...
                else
                {
                    ...
                    printk( "[futengfei] %s: Found Function 0x%02x - Ignored.\n", __func__, rmi_fd.functionNum & 0xff);
                }
            }
            else
            {
                ...
                printk("[futengfei] %s:   Found End of PDT.\n",				__func__);
                break;
            }
        }
        else
        {
            ...
            let it continue. */
            printk( "[futengfei] %s: Read Error %d when reading next PDT entry - ending PDT scan.",	__func__, retval);
            break;
        }
    }
    ...
    if (rmi_sensor_dev)
    {
        ...
        printk( "[futengfei] %s: Registered sensor drivers.\n", __func__);
        retval = 0;
    }
    else
    {
        printk( "[futengfei] %s: Failed to find sensor. PDT contained %d entries.\n", __func__, pdt_entry_count);
        retval = -ENODEV;
    }
...
{
    printk( "[futengfei]%s:come into ->rmi_sensor_teardown	= synaptics_touchpad_gpio_teardown, NO SYNAPTICS GPIO TEARDOWN!\n", __func__);
}
...
{
    int retval;
    retval = gpio_request(SYNAPTICS_TS_IRQ, "rmi4_attn");
    printk("[futengfei] come into rmi_sensor_setup->synaptics_touchpad_gpio_setup1 retval=%d\n", retval);
    if (retval)
    {
        printk(KERN_ERR "[futengfei]%s: Failed to obtain touchpad IRQ %d. Code: %d.\n", __func__, SYNAPTICS_TS_IRQ, retval);
    }
    else
    {
        gpio_direction_output(SYNAPTICS_TS_IRQ, 1);
        //		retval = gpio_direction_input(SYNAPTICS_TS_IRQ);
        if (retval)
        {
            printk(KERN_ERR "[futengfei]%s: Failed to setup touchpad IRQ %d. Code: %d.\n", __func__, SYNAPTICS_TS_IRQ, retval);
            //	// TODO: Free the GPIO
        }
    }

    gpio_tlmm_config(GPIO_CFG(SYNAPTICS_TS_IRQ, 0,	GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
    gpio_tlmm_config(GPIO_CFG(FT5X06_RESET_GPIO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_16MA), GPIO_CFG_ENABLE);

    return retval;
}

static unsigned char synaptics_button_codes[] = {KEY_HOME, KEY_BACK,};
...
static struct rmi_sensordata synaptics_sensordata =
{
    ...
    .rmi_sensor_setup	= synaptics_touchpad_gpio_setup,
    .rmi_sensor_teardown	= synaptics_touchpad_gpio_teardown,
};
...
static struct rmi_i2c_platformdata synaptics_platformdata =
{
    ...
    .irq_type = /*IORESOURCE_IRQ_LOWLEVEL,*/IORESOURCE_IRQ_LOWEDGE,
    .sensordata = &synaptics_sensordata,
};

static struct i2c_board_info qcomnewtouch[]  =
{
    {
