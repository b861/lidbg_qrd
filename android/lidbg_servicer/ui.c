...
static int key_queue[256][2], key_queue_len = 0; //add a data to store the keyup or keydown status
...
static int input_callback(int fd, short revents, void *data)
{
    ...
    const int queue_max = sizeof(key_queue) / sizeof(key_queue[0]);
        if (ev.value >= 0 && key_queue_len < queue_max) {
            key_queue[key_queue_len][0] = ev.code;
            key_queue[key_queue_len++][1] = ev.value;
...
int ui_wait_key(int * key_status)
{
    pthread_mutex_lock(&key_queue_mutex);
    ...
    if (key_queue_len > 0) {
        key = key_queue[0][0];
        *key_status = key_queue[0][1];        
        memcpy(&key_queue[0][0], &key_queue[1][0], sizeof(key_queue[0]) * --key_queue_len);
    }
    pthread_mutex_unlock(&key_queue_mutex);
    return key;
}
