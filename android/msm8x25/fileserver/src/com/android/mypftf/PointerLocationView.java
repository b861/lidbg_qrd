	...
	public void addTouchEvent(MotionEvent event)
	{
		synchronized (mPointers)
		{
			...
			if ((action & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_DOWN)
			{
				...
				final int id = event.getPointerId(index); // 对于每个触控的点的细节，我们可以通过一个循环执行getPointerId方法获取索引
			...
			final int NI = event.getPointerCount();// 获取触控点的数量，比如2则可能是两个手指同时按压屏幕
			...
			for (int i = 0; i < NI; i++)
			{
				...
				ps.mCurWidth = (int) (ps.mCurSize * (getWidth() / 3))/7;
			}
